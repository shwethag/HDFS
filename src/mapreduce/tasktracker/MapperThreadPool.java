...
class MapWorker implements Runnable{
	...
	public void handleMapFunction(ByteString blockString){
		...
		try {
			...
			File mapReduceJarFile = new File("./lib/mapreduce.jar");
			URLClassLoader child = new URLClassLoader (new URL [] {mapReduceJarFile.toURL()}, System.class.getClassLoader());
			Class classToLoad = Class.forName ("mapreduce.mapper.Mapper", true, child);
			Method method = classToLoad.getDeclaredMethod ("map");
			Object instance = classToLoad.newInstance ();
			
			
			String [] lines = blockString.toStringUtf8().split(NEWLINE);
			for (String line : lines) {
				String res=method.invoke(instance, line);
			}
	...
	public void executeMapTask(){
		...
		for (DataNodeLocation daLocation : blockLocations.getLocationsList()) {
			...
			try {
				...
				System.out.println("INFO: Read block successfully.. " + blockLocations.getBlockNumber());
				break;
		...
		//Once done upload the result file to HDFS and return the file information in mapTaskStatus
	...
	public void run() {
		...
		synchronized (MapperThreadPool.lock) {
			...
			System.out.println("INFO: reducing available count of mapthread to: "+(MapperThreadPool.availableCount-1));
		...
		synchronized (MapperThreadPool.lock) {
			...
			System.out.println("INFO: Increasing available count of mapthread to: "+(MapperThreadPool.availableCount+1));
			MapperThreadPool.availableCount++;
			//Flip completed flag in the map to true and move the entry set from activeMap to completedMap
			int index=-1,requiredIndex=-1;
			MapTaskStatus mapStatus=null;
			for (MapTaskStatus mapTaskStatus : MapperThreadPool.activeThreadTask) {
				index++;
				if(mapTaskStatus.getJobId() == mapTaskInfo.getJobId() && mapTaskStatus.getTaskId()==mapTaskInfo.getTaskId()){
			...
			System.out.println("INFO: Moving Completed Task from ActiveThreadList to completedThreadList ");
